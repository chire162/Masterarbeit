...
int rhs (double t, const double spin[], double f[], void * _params){
    ...
    */
    for (int i = 0; i < params.len_energies; i++){
        ...
        f[3 * i + 0] = 2.0 * ( -params.Delta_y * spin[3 * i + 2] -  params.energies[i]  * spin[3 * i + 1]);
        f[3 * i + 1] = 2.0 * (params.energies[i] * spin[3 * i + 0] + params.Delta_x * spin[3 * i +2]);
        f[3 * i + 2] = 2.0 * (-params.Delta_x * spin[3 * i + 1] + params.Delta_y * spin[3 * i + 0]);
    }
    return GSL_SUCCESS;
}